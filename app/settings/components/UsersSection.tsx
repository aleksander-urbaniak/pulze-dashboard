...
function FooterBar({ count }: { count: number }) {
  return (
    <footer className="flex items-center justify-between border-t border-border bg-base/40 px-4 py-4 md:px-6">
      <p className="text-sm font-semibold text-muted">Showing {count ? "1" : "0"} to {count} of {count}</p>
      <div className="flex items-center gap-2">
        <button type="button" className={clsx(settingsMutedButton, "h-10 px-5 py-0 opacity-60")} disabled>
          Previous
        </button>
        <span className="text-sm font-semibold text-text">Page 1 of 1</span>
        <button type="button" className={clsx(settingsMutedButton, "h-10 px-5 py-0 opacity-60")} disabled>
          Next
        </button>
      </div>
    ...
    return (
      <section className="space-y-4">
        <PageSectionHeader
          ...
          right={headerRight}
        />
        <p className="text-sm text-muted">Your role does not include user management.</p>
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <PageSectionHeader
        ...
        right={headerRight}
      />
      {userStatus ? <p className="text-xs text-muted">{userStatus}</p> : null}
      ...
      <article className={clsx(settingsShellCard, "overflow-hidden")}>
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4 md:px-6">
          ...
          <div className="flex w-full flex-wrap justify-end gap-2 md:w-auto">
            <label className="relative min-w-[220px] flex-1 md:flex-none">
              <FontAwesomeIcon icon={faMagnifyingGlass} className="absolute left-4 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted" />
        ...
        <div className="grid grid-cols-[minmax(320px,1fr)_170px_210px_220px] border-b border-border bg-base/40 px-5 py-3 text-[11px] font-bold uppercase tracking-[0.26em] text-muted md:px-6">
          <span>User</span>
          <span className="border-l border-border text-center">Role</span>
          <span className="border-l border-border text-center">2FA</span>
          <span className="border-l border-border text-center">Actions</span>
        </div>
        <div className="divide-y divide-border">
            ...
            return (
              <div key={u.id} className="grid grid-cols-[minmax(320px,1fr)_170px_210px_220px] items-center px-5 py-4 md:px-6">
                ...
                <div className="flex items-center justify-center border-l border-border">
                  <span className="inline-flex w-fit rounded-lg bg-accentSoft px-3 py-1 text-sm font-semibold text-accent">{u.role}</span>
                </div>
                <div className="flex items-center justify-center border-l border-border">
                ...
                <div className="flex items-center justify-center gap-1.5 border-l border-border">
                  {editing ? (
                    <Fragment>
                    ...
                    <Fragment>
                      <button type="button" onClick={() => onDeleteUser(u.id)} className={clsx(settingsMutedButton, "inline-flex h-9 w-9 items-center justify-center px-0 py-0 leading-none text-muted hover:text-rose-300")} title="Delete user"><FontAwesomeIcon icon={faTrashCan} className="h-3.5 w-3.5" /></button>
                      <button type="button" onClick={() => setEditingUserId(u.id)} className={clsx(settingsMutedButton, "inline-flex h-9 w-9 items-center justify-center px-0 py-0 leading-none text-muted")} title="Edit user"><FontAwesomeIcon icon={faEllipsisVertical} className="h-3.5 w-3.5" /></button>
                    </Fragment>
                  )}
                </div>
                <div
                  className={clsx(
                    ...
                    editing ? "mt-4 max-h-[520px] border-t border-border pt-4 opacity-100" : "max-h-0 border-t border-transparent pt-0 opacity-0"
                  )}
                >
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <Field label="Reset Password">
                      <div className="relative">
                        <FontAwesomeIcon icon={faKey} className="absolute left-4 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted" />
          ...
          {filteredUsers.length === 0 ? <div className="px-5 py-8 text-sm text-muted md:px-6">No users found.</div> : null}
        </div>
      ...
      <article className={clsx(settingsShellCard, "overflow-hidden")}>
        ...
        <div className="grid grid-cols-[minmax(280px,1fr)_120px_180px_220px] border-b border-border bg-base/40 px-5 py-3 text-[11px] font-bold uppercase tracking-[0.26em] text-muted md:px-6">
          <span>Group</span>
          <span className="border-l border-border text-center">Members</span>
          <span className="border-l border-border text-center">Default role</span>
          <span className="border-l border-border text-center">Actions</span>
        </div>
        <div className="divide-y divide-border">
            ...
            return (
              <div key={g.id} className="grid grid-cols-[minmax(280px,1fr)_120px_180px_220px] items-center px-5 py-4 md:px-6">
                <div className="flex items-center gap-2.5">
                  <FontAwesomeIcon icon={faUsers} className="h-3.5 w-3.5 text-muted" />
                  <span className="text-xl font-semibold text-text">{g.name}</span>
                </div>
                <div className="flex items-center justify-center border-l border-border">
                  <span className="text-lg text-text">{groupMembers.get(g.id) ?? 0}</span>
                </div>
                <div className="flex items-center justify-center border-l border-border">
                  <span className="inline-flex w-fit rounded-lg bg-accentSoft px-3 py-1 text-sm font-semibold text-accent">
                    {g.defaultRole}
                  </span>
                </div>
                <div className="flex items-center justify-center gap-2 border-l border-border">
                  {editing ? (
                    ...
                    <Fragment>
                      <button
                        ...
                        className={clsx(settingsMutedButton, "inline-flex h-9 w-9 items-center justify-center px-0 py-0 leading-none text-muted hover:text-rose-300")}
                        title="Delete group"
                      >
                        <FontAwesomeIcon icon={faTrashCan} className="h-3.5 w-3.5" />
                      </button>
                      <button type="button" onClick={() => { setEditingGroupId(g.id); setGroupDrafts((p) => ({ ...p, [g.id]: { name: g.name, defaultRole: g.defaultRole } })); }} className={clsx(settingsMutedButton, "inline-flex h-9 w-9 items-center justify-center px-0 py-0 leading-none text-muted")} title="Edit group">
                ...
                <div
                  className={clsx(
                    "col-span-4 grid overflow-hidden transition-all duration-300 [transition-timing-function:linear]",
                    editing ? "mt-4 max-h-[360px] border-t border-border pt-4 opacity-100" : "max-h-0 border-t border-transparent pt-0 opacity-0"
                  )}
                >
                  <div className="grid gap-3 md:grid-cols-2">
          ...
          {filteredGroups.length === 0 ? <div className="px-5 py-8 text-sm text-muted md:px-6">No groups found.</div> : null}
        </div>
      ...
      {isCreateUserModalOpen ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#030814]/55 px-4 backdrop-blur-sm">
          <div className="glass-shell w-full max-w-xl rounded-[20px] border p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Field label="First Name"><div className="relative"><FontAwesomeIcon icon={faCircleUser} className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input value={newUser.firstName} onChange={(e) => setNewUser((p) => ({ ...p, firstName: e.target.value }))} className={clsx(settingsFieldClass, "pl-11")} placeholder="First name" /></div></Field>
              <Field label="Last Name"><div className="relative"><FontAwesomeIcon icon={faCircleUser} className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input value={newUser.lastName} onChange={(e) => setNewUser((p) => ({ ...p, lastName: e.target.value }))} className={clsx(settingsFieldClass, "pl-11")} placeholder="Last name" /></div></Field>
              <Field label="Username"><div className="relative"><FontAwesomeIcon icon={faCircleUser} className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input value={newUser.username} onChange={(e) => setNewUser((p) => ({ ...p, username: e.target.value }))} className={clsx(settingsFieldClass, "pl-11")} placeholder="Username" /></div></Field>
              ...
              <Field label="Password"><div className="relative"><FontAwesomeIcon icon={faKey} className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input type="password" value={newUser.password} onChange={(e) => setNewUser((p) => ({ ...p, password: e.target.value }))} className={clsx(settingsFieldClass, "pl-11")} placeholder="Password" /></div></Field>
            </div>
      ...
      {isCreateGroupModalOpen ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#030814]/55 px-4 backdrop-blur-sm">
          <div className="glass-shell w-full max-w-lg rounded-[20px] border p-5">
            <div className="mb-4 flex items-center justify-between">
